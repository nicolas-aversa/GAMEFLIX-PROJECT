...
export default function UnpublishGame({ games, onSuccess, onError }) {
  const handleStatusChange = async (gameId) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/games/${gameId}/status`, {
        ...
        body: JSON.stringify({ status: 'Despublicado' })
      });
      ...
      await response.json();
      onSuccess('Juego despublicado exitosamente');
      setTimeout(() => {
        onSuccess('');
      }, 5000);
    } catch (error) {
      console.error('Error al despublicar el juego:', error);
      onError(`Hubo un error al despublicar el juego: ${error.message}`);
    }
  };
      ...
      <ul>
        {games.map((game) => (
          <li key={game._id} className="mb-4 flex justify-between items-center bg-gradient-to-r from-purple-900 to-purple-1000 px-8 py-2 rounded-full transition-all duration-300 ease-out p-6 rounded-lg shadow-lg">
            ...
            <button
              type="button"
              className="bg-orange-600 text-white rounded-2xl py-1.5 px-3 hover:bg-orange-800 font-medium shadow-lg hover:scale-105 transition-transform duration-200"
